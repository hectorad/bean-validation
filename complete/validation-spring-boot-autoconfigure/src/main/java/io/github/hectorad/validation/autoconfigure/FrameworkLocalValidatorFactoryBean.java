...
public class FrameworkLocalValidatorFactoryBean extends LocalValidatorFactoryBean {

    private final boolean validationEnabled;

    private final List<ValidationBypassStrategy> bypassStrategies;

    private final ExecutableValidator executableValidator = new FrameworkExecutableValidator();

    public FrameworkLocalValidatorFactoryBean(boolean validationEnabled, List<ValidationBypassStrategy> bypassStrategies) {
        this.validationEnabled = validationEnabled;
        this.bypassStrategies = (bypassStrategies == null) ? List.of() : List.copyOf(bypassStrategies);
    }
    ...
    boolean shouldBypassValidation() {
        return !validationEnabled || bypassStrategies.stream().anyMatch(ValidationBypassStrategy::shouldBypassValidation);
    }

    private ExecutableValidator delegateExecutableValidator() {
        return FrameworkLocalValidatorFactoryBean.super.forExecutables();
    }

    private final class FrameworkExecutableValidator implements ExecutableValidator {
