...
public class ConfigDrivenConstraintMappingContributor implements ConstraintMappingContributor {
	...
	private final Map<String, Map<String, ConstraintOverrideSet>> overrideIndex;
	...
	public ConfigDrivenConstraintMappingContributor(
		...
		this(mergedOverrides, generatedClassMetadataCache, constraintMergeService, true);
	}
	...
	public ConfigDrivenConstraintMappingContributor(
		...
		this.overrideIndex = buildOverrideIndex(mergedOverrides);
	...
	public void createConstraintMappings(ConstraintMappingBuilder builder) {
		for (ResolvedClassMapping resolvedClassMapping : resolvedClassMappings) {
			...
			Map<String, ConstraintOverrideSet> fieldOverrides =
				overrideIndex.getOrDefault(resolvedClassMapping.className(), Map.of());

			for (ResolvedFieldMapping resolvedFieldMapping : resolvedClassMapping.fields()) {
				try {
					ConstraintOverrideSet overrideSet = fieldOverrides.get(resolvedFieldMapping.fieldName());
	...
	/**
	 * Builds a nested index of class → field → ConstraintOverrideSet from pre-merged overrides.
	 */
	private static Map<String, Map<String, ConstraintOverrideSet>> buildOverrideIndex(
		List<ClassValidationOverride> mergedOverrides
	) {
		Map<String, Map<String, ConstraintOverrideSet>> index = new HashMap<>();
		for (ClassValidationOverride classOverride : mergedOverrides) {
			if (classOverride.className() == null) {
				continue;
			}
			Map<String, ConstraintOverrideSet> fieldIndex =
				index.computeIfAbsent(classOverride.className(), k -> new HashMap<>());
			for (FieldValidationOverride fieldOverride : classOverride.fields()) {
				if (fieldOverride.fieldName() == null) {
					continue;
				}
				fieldIndex.putIfAbsent(fieldOverride.fieldName(), fieldOverride.constraints());
			}
		}
		return index;
	}
